  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[2.5rem] p-10 w-full max-w-md shadow-2xl relative">
        ...
        <h2 className="text-2xl font-extrabold mb-6">Registrar Falta {isDocente ? 'Docente' : 'Estudiante'}</h2>
        <form onSubmit={onSubmit} className="space-y-6">
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-700 uppercase">{isDocente ? 'Docente' : 'Estudiante'}</label>
            <select required value={isDocente ? data.teacherId : data.studentId} onChange={e => isDocente ? setData({...data, teacherId: e.target.value}) : setData({...data, studentId: e.target.value})} className="w-full px-6 py-4 bg-[#F1F1F1] border-2 border-[#D9D9D9] rounded-2xl outline-none focus:border-[#24157A]">
          ...
          <div className="grid grid-cols-2 gap-4">
            <input type="date" required value={data.date} onChange={e => setData({...data, date: e.target.value})} className="w-full px-6 py-4 bg-[#F1F1F1] border-2 border-[#D9D9D9] rounded-2xl outline-none focus:border-[#24157A]" />
